...
image:
  url: "https://dev-tools-blog-images.fly.storage.tigris.dev/blog/har-to-yaml-generate-api-regression-tests-for-ci-github-actions-example/main-image.webp"
...
![A simple pipeline diagram showing four boxes connected left to right: “Browser HAR capture” -> “Convert HAR to YAML flow” -> “Commit YAML in Git” -> “GitHub Actions runs flow and uploads JUnit report”.](https://dev-tools-blog-images.fly.storage.tigris.dev/blog/har-to-yaml-generate-api-regression-tests-for-ci-github-actions-example/image-0.webp)
...
- Request A (Login) returns `access_token` in the response body
- Request B references `{{Login.response.body.access_token}}` in its Authorization header
- Request C uses an `id` from Request B's response: `{{CreateResource.response.body.id}}`

Below is an example using the DevTools YAML format, showing request chaining and validation:
  ...
  - name: UserFlow
    steps:
      ...
      - js:
          ...
          code: |
            export default function(ctx) {
              if (ctx.GetCurrentUser?.response?.status !== 200) throw new Error("Expected 200");
              if (!ctx.GetCurrentUser?.response?.body?.id) throw new Error("Missing id");
              return { validated: true };
            }
          depends_on: GetCurrentUser
```

What matters for CI is not the exact key names, it is the discipline:

- **reference node outputs directly** (e.g. `{{Login.response.body.access_token}}`)
- **reuse everywhere**
- **validate on behavior, not incidental fields** (using `js:` nodes)
...
If you are migrating from Newman, the biggest mental shift is that your "collection + environments" becomes "flows + env variables and node output references", and the YAML is the unit of review.
