...
image:
  url: "https://dev-tools-blog-images.fly.storage.tigris.dev/blog/yaml-api-test-file-structure-conventions-for-readable-git-diffs/main-image.webp"
...
![A GitHub pull request diff view showing a YAML API flow file with a small, clean change in one step, highlighting stable key ordering and minimal diff noise.](https://dev-tools-blog-images.fly.storage.tigris.dev/blog/yaml-api-test-file-structure-conventions-for-readable-git-diffs/image-0.webp)
...
- Step type (`request:`, `js:`, `if:`, `for:`, `for_each:`)
- `name` (inside the typed node)
- `method`, `url` (for requests)
- `headers`
- `body` / `query_params`
- `depends_on`
- `code` (for `js:` nodes)
...
- references between steps (via `{{CreateUser.response.body.id}}`)
- targeted retries
- selective execution
...
- request node
- `js:` validation node (validates response and optionally returns computed values)
- next request node references upstream values via `{{NodeName.response.body.field}}`
...
- Values from earlier steps are referenced directly via `{{Login.response.body.access_token}}`.
- Keep naming consistent (`Login`, `GetProfile`). Avoid abbreviations like `L`, `GP`, etc.
...
- reference specific fields: `{{CreateUser.response.body.id}}`, `{{GetProfile.response.headers.etag}}`
- use `js:` nodes to compute derived values when necessary
- avoid returning entire response bodies from `js:` nodes unless you are snapshot testing
...
- Keep `js:` validation nodes close to the request they validate.
