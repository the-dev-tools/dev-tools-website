...
image:
  url: "https://kccqmbkylzrrhibpxtbk.supabase.co/storage/v1/object/public/public-user-assets/b460f601-795d-47ba-abf4-41fb76ccea6f/2fc8ce22-24b5-40b8-af6c-adbd41656eca/main-image.webp"
...
- The test is **data flow driven**. You capture `project_id` once and reuse it.
- Variables are explicit (`base_url`, `token`), not implicit globals.
- Assertions are declared as data, which is easier to diff in PRs.
...
### Capture from JSON response

You already saw capturing `$.id`. Here is a slightly more realistic pattern with nested data and arrays.
...
```yaml
url: "${base_url}/v1/users/${user_id}/roles/${first_role}"
```

### Avoid implicit coupling

If a step needs a value, make it obvious by keeping the capture adjacent to the producer request and using stable step IDs.

A migration tip: if your Postman suite uses `pm.collectionVariables.set("x", ...)` all over the place, you likely have hidden coupling. Extract those into explicit captures and references, or your YAML will become the same “spooky action at a distance”, just in a different file.
...
### Map the 80 percent case to declarative assertions

Most suites check:

- status codes
- presence/type of fields
- equality against captured values
- simple header checks

Those should become YAML rules. The exact syntax depends on your runner, but your goal is the same: **assert invariants that survive harmless response evolution**.
...
expect:
  ...
  json:
    - path: $.id
      equals: "${project_id}"
    - path: $.createdAt
      matches: "^\\d{4}-\\d{2}-\\d{2}T"  # timestamp shape, not exact value
```

### When you still need JavaScript

Some assertions are easier with code:

- canonicalizing arrays before comparing
- validating signatures
- computing derived values

DevTools supports JavaScript nodes in flows (use them sparingly). Keep JS logic small and deterministic, and prefer it for extraction/normalization rather than for controlling the entire test.

A reasonable split is:

- YAML for request/response and the assertion surface
- JS for parsing and producing a small set of derived values
...
Same structure, different endpoint. Capture the access token and pass it.
