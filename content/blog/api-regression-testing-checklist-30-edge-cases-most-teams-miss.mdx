...
- **Deterministic diffs**: In Postman, you tend to accumulate “test scripts” and implicit collection state. Reviewers rarely catch subtle logic changes. With YAML in Git, a change from `status: 200` to `status: [200, 207]` is explicit.
- **Request chaining is visible**: Captures (IDs, tokens, cursors) live next to the request that produced them, which makes drift bugs easier to diagnose in PRs.
