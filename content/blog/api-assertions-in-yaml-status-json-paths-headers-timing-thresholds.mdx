...
- Give each step a clear `name` inside its typed node
- Validate responses with `js:` nodes before using values downstream
- Reference upstream values directly with `{{NodeName.response.body.field}}`
...
For the full guide to YAML-native API testing, including syntax patterns, variable passing, environments, and Git workflows, see: [YAML-Native API Testing: Define, Version, and Run Tests as Code](/guides/yaml-native-api-testing/).