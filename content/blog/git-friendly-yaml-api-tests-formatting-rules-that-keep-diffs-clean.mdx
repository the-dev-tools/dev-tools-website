...
- You “touch” many steps at once by renaming IDs or moving extraction logic away from where it’s used.
...
- Step identifier (`id` or `name`)
- Request definition (`request`, `method`, `url`, `headers`, `query`, `body`)
- Variable capture (`extract`, `capture`)
- Assertions (`assert`)
...
- Use stable, machine-friendly IDs (`create_user`, not `Create user step`).
- Don’t rename IDs for “readability” unless the behavior changed.
...
| `|` for multi-line bodies | Pretty JSON, stable whitespace | Reviewable payload changes |
