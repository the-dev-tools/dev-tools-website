...
image:
  url: "https://dev-tools-blog-images.fly.storage.tigris.dev/blog/how-to-redact-har-files-safely-keep-tests-shareable-remove-secrets/main-image.webp"
...
![Simple pipeline diagram showing: raw HAR file on a developer laptop flows into a sanitizer step, then into YAML API flows stored in Git, then executed by CI runners with secrets injected from the CI environment.](https://dev-tools-blog-images.fly.storage.tigris.dev/blog/how-to-redact-har-files-safely-keep-tests-shareable-remove-secrets/image-0.webp)
...
- Use it on subsequent requests via `{{Login.response.body.access_token}}`
...
- Secrets become obvious placeholders (`{{API_TOKEN}}`), not opaque values buried in exported JSON.
- Request chaining is explicit via node output references (`{{NodeName.response.body.field}}`), so you don't need to preserve browser cookies from the HAR.
...
- Replace cookie-based auth with a login request step and reference the token from its response body.
