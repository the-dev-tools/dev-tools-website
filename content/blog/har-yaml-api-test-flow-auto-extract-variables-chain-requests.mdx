...
image:
  url: "https://kccqmbkylzrrhibpxtbk.supabase.co/storage/v1/object/public/public-user-assets/b460f601-795d-47ba-abf4-41fb76ccea6f/92447b4a-b16e-4e41-83e8-8586010b984d/main-image.webp"
...
If you can reproduce a user workflow in the browser, you can usually capture the underlying HTTP traffic. The hard part is turning that capture into a deterministic, reviewable API test that survives refactors, runs in CI, and does not embed secrets.

A good HAR → YAML pipeline does two things well:

- **Auto-extract variables** (tokens, IDs, CSRF values, cursors) so the flow is runnable, not just a static replay.
- **Chain requests explicitly** so every dependency is visible in Git diffs and PR review.
...
![Diagram showing a pipeline: focused browser workflow captured into a HAR file, sanitized/redacted, converted into a YAML flow with extracted variables, committed to Git for PR review, then executed in CI with JUnit/log artifacts.](https://kccqmbkylzrrhibpxtbk.supabase.co/storage/v1/object/public/public-user-assets/b460f601-795d-47ba-abf4-41fb76ccea6f/92447b4a-b16e-4e41-83e8-8586010b984d/image-0.webp)
...
The key is: **prefer chaining from the API response that “created” the value**, not from a later request where it merely appeared.
...
Below are examples showing the *shape* of common chaining patterns. Use them as a template for structuring your flow, and adapt field names to the exact DevTools YAML schema you are using.

### Pattern 1: Login → capture token → authenticated calls

```yaml
# flow.yaml (simplified)
vars:
  base_url: ${BASE_URL}
...
- Token capture is adjacent to the login step.
- Downstream steps reference the token via a single variable.
- Assertions validate contract-level behavior (status, presence), not a full response snapshot.

### Pattern 2: Create resource → capture ID from `Location` → fetch

Many APIs return the canonical URL for the created resource in a `Location` header. This is ideal for chaining because it decouples your test from response-body shape changes.
...
- **Stable step IDs** that do not change when you re-record (avoid auto-generated names that reorder).
- **Adjacent capture**: capture variables right after the response that introduces them.
...
**How does auto-extract variable mapping actually help?** It turns “replayed traffic” into an executable chain by detecting values that originate in one response and are reused later (tokens, IDs, CSRF). You still review and tighten the mapping so it is explicit and stable.

**Should I rely on cookies or pass tokens explicitly?** For API regression tests, explicit chaining (capture token, set header) is usually easier to review and debug than implicit cookie state. Cookies can still be useful, but keep the dependency visible in YAML.
