...
- A fixed ordering (by flow file path, then by step name)
...
Request chaining is where UI tools often hide complexity: token references, ID propagation, correlation, and environment switching. In YAML, you want this explicit so PR reviewers can see the data flow.

Here is a flow showing the pattern: reference node outputs directly and validate with `js:` nodes on stable contracts.

```yaml
workspace_name: Payments Smoke
  ...
  - name: PaymentsSmoke
    steps:
      ...
      - js:
          ...
          code: |
            export default function(ctx) {
              if (ctx.Auth?.response?.status !== 200) throw new Error("Auth failed");
              if (!ctx.Auth?.response?.body?.access_token) throw new Error("No token");
      ...
      - js:
          ...
          code: |
            export default function(ctx) {
              if (ctx.CreateCharge?.response?.status !== 201) throw new Error("Expected 201");
              if (ctx.CreateCharge?.response?.body?.currency !== 'USD') throw new Error("Expected USD");
              return { validated: true };
            }
          depends_on: CreateCharge

      - request:
          name: GetCharge
          method: GET
          url: '{{BASE_URL}}/charges/{{CreateCharge.response.body.id}}'
          headers:
            Authorization: 'Bearer {{Auth.response.body.access_token}}'
          depends_on: CreateCharge

      - js:
          name: ValidateGetCharge
          code: |
            export default function(ctx) {
              if (ctx.GetCharge?.response?.status !== 200) throw new Error("Expected 200");
              if (ctx.GetCharge?.response?.body?.id !== ctx.CreateCharge?.response?.body?.id) {
                throw new Error("Charge ID mismatch");
              }
              return { validated: true };
            }
          depends_on: GetCharge
```

The review principle stays the same: chaining should be obvious in the diff, and every dependency between steps is visible via `depends_on` and `{{NodeName.response.body.field}}` references.
...
- schema invariants
- id propagation (reference from the creating node, compare later)

This is where YAML flows tend to be clearer than Postman script assertions because request definitions are declarative, validation logic is isolated in `js:` nodes, and reviewers do not have to interpret scattered ad hoc JavaScript.
...
**How do YAML flows help with code review compared to Postman collections?** YAML is human-readable and produces cleaner diffs. Reviewers can see exactly which request, validation node, or chained reference changed, without reverse-engineering an exported collection.

**How should we handle secrets in Git workflows for API tests?** Keep secrets out of YAML, inject them via CI secrets or environment variables, and prefer request chaining (referencing node outputs via `{{NodeName.response.body.token}}`) to fetch short-lived tokens. If your suite starts from HAR captures, redact aggressively before anything leaves a developer machine.
