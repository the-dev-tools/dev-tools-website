...
- Use it explicitly in subsequent mutating requests via `{{NodeName.response.body.field}}`.

If you want more concrete examples of chaining node outputs from real HAR traffic, see: [HAR → YAML API Test Flow: Auto-Extract Variables + Chain Requests](https://dev.tools/blog/har-yaml-api-test-flow-auto-extract-variables-chain-requests/).
...
The YAML flows this pipeline produces are [end-to-end API tests](/blog/end-to-end-api-testing-the-complete-guide/): chained requests with real data flowing between steps. For a deeper look at structuring these multi-step workflows, see the complete guide.
