...
- Name test cases using **stable identifiers** (step names, flow names). This makes diffs in historical runs meaningful.
...
### Use stable step names and reference node outputs directly
      ...
      - js:
          ...
          code: |
            export default function(ctx) {
              if (ctx.AuthToken?.response?.status !== 200) throw new Error("Auth failed");
              if (!ctx.AuthToken?.response?.body?.access_token) throw new Error("No token");
...
The audit advantage is that a failure can be described as "`GetProfile` returned 401" rather than "request #17 failed."
...
**How do we keep artifacts useful without leaking data?** Keep logs high-signal (step names, status codes, assertion diffs) and make deep HTTP traces opt-in with automated redaction.
